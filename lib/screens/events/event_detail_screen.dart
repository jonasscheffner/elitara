...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  List<Widget> _buildActionButtons(
    ...
    int currentWaitlistCount,
    Future<void> Function(String) updateWaitlist,
  ) {
    final isHost = currentUser?.uid == ev.host;
    final isCoHost =
        currentUser != null && ev.coHosts.contains(currentUser.uid);
    ...
    buttons.add(ElevatedButton.icon(
      onPressed: () {
        showGeneralDialog(
          ...
          barrierLabel: locale.translate(section, 'participant_list'),
      ...
      label: Text(locale.translate(section, 'participant_list')),
    ...
    if (ev.waitlistEnabled && isHost) {
      buttons.add(ElevatedButton.icon(
        onPressed: () async {
          final currentParticipants = ev.participants.length;
              ...
              return Stack(
                children: [
                  BackdropFilter(
                    filter: ImageFilter.blur(sigmaX: 5, sigmaY: 5),
                    child: Container(
                        color: const Color(0x80000000).withOpacity(0)),
                  ),
                  WaitlistDialog(
                    eventId: ev.id,
                    eventTitle: ev.title,
      ...
      buttons.add(ElevatedButton.icon(
        onPressed: () async {
          ...
          showGeneralDialog(
            context: context,
            barrierDismissible: true,
            barrierLabel: locale.translate(section, 'invite_users'),
            pageBuilder: (c, a1, a2) => Stack(
              children: [
                BackdropFilter(
                  filter: ImageFilter.blur(sigmaX: 5, sigmaY: 5),
                  child:
                      Container(color: const Color(0x80000000).withOpacity(0)),
                ),
                Center(
        ...
        label: Text(locale.translate(section, 'invite_users')),
        style: ElevatedButton.styleFrom(
          padding: const EdgeInsets.symmetric(vertical: 14),
    ...
    if (showEdit) {
      buttons.add(ElevatedButton.icon(
        onPressed: () {
          Navigator.pushNamed(context, '/editEvent', arguments: ev.id);
        },
        icon: const Icon(Icons.edit),
        label: Text(locale.translate(section, 'edit_event')),
        style: ElevatedButton.styleFrom(
          padding: const EdgeInsets.symmetric(vertical: 14),
      ...
      buttons.add(ElevatedButton.icon(
        onPressed: () async {
          final confirm = await showGeneralDialog<bool>(
            ...
            barrierDismissible: true,
            barrierLabel:
                locale.translate(section, 'cancel_confirmation_dialog.title'),
            pageBuilder: (c, a1, a2) => Stack(
              children: [
                BackdropFilter(
                  filter: ImageFilter.blur(sigmaX: 5, sigmaY: 5),
                ...
                Center(
                  child: AlertDialog(
                    title: Text(locale.translate(
                        section, 'cancel_confirmation_dialog.title')),
                    content: Text(locale.translate(
                        section, 'cancel_confirmation_dialog.content')),
                    actions: [
                      TextButton(
                          onPressed: () => Navigator.pop(c, false),
                          child: Text(locale.translate(
                              section, 'cancel_confirmation_dialog.no'))),
                      TextButton(
                          onPressed: () => Navigator.pop(c, true),
                          child: Text(locale.translate(
                              section, 'cancel_confirmation_dialog.yes'))),
                    ],
                  ),
                ),
              ],
        ...
        label: Text(locale.translate(section, 'cancel_event')),
        style: ElevatedButton.styleFrom(
          padding: const EdgeInsets.symmetric(vertical: 14),
    ...
    final isOnWaitlist = currentUser != null &&
        ev.waitlist.any((e) => e['uid'] == currentUser.uid);

    if (!isJoined) {
      if (ev.accessType == AccessType.public &&
          (ev.participantLimit == null ||
              ev.participants.length < ev.participantLimit!)) {
        buttons.add(ElevatedButton.icon(
          onPressed: () async {
            await _eventService.registerForEvent(ev.id, currentUser!.uid);
            AppSnackBar.show(
              context,
        ...
        buttons.add(const SizedBox(height: 12));
      } else if (ev.participantLimit != null &&
          ev.participants.length >= ev.participantLimit! &&
          ev.waitlistEnabled) {
        if (ev.waitlistLimit != null &&
            currentWaitlistCount >= ev.waitlistLimit!) {
          buttons.add(Text(
          ...
          buttons.add(ElevatedButton(
            ...
            child: Text(locale.translate(section, 'leave_waitlist')),
          ));
          buttons.add(const SizedBox(height: 12));
        } else {
          buttons.add(ElevatedButton(
            onPressed: () async {
              ...
              AppSnackBar.show(
                context,
                locale.translate(section, 'waitlist_registered'),
                type: SnackBarType.success,
              );
            },
            style: ElevatedButton.styleFrom(
            ...
            child: Text(locale.translate(section, 'join_waitlist')),
          ));
          buttons.add(const SizedBox(height: 12));
        }
      }
    ...
    if (isJoined && !isHost) {
      buttons.add(ElevatedButton.icon(
        onPressed: () async {
          await _eventService.leaveEvent(ev.id, currentUser!.uid);
          AppSnackBar.show(
            context,
